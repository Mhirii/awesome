...
var dataFile = filepath.Join(configDir, "dualData.toml")
...
func setWallpaper(data *DualData, wallpaperName string) {
	wallpaperDir := filepath.Join(configDir, "wallpapers/dual", wallpaperName)
	wallpaperPath := filepath.Join(wallpaperDir, "wallpaper.png")

	cmd := exec.Command("feh", "--bg-fill", "--no-xinerama", wallpaperPath)
	err := cmd.Run()
	if err != nil {
...
func getCurrentWallpaper(data *DualData) {
...
func updateAvailableWallpapersList(data *DualData) {
	availableWallpapers := make([]string, 0)

	files, err := ioutil.ReadDir(filepath.Join(configDir, "wallpapers/dual"))
	if err != nil {
...
func cycleWallpapers(data *DualData) {
